...
class Ui_Principal(object):  # {{{1
    def setupUi(self, Principal):
        ...
        Principal.resize(QtCore.QSize(QtCore.QRect(
            0, 0, 780, 550).size()).expandedTo(Principal.minimumSizeHint()))
        ...
        sizePolicy.setHeightForWidth(
            self.Boton_Salir.sizePolicy().hasHeightForWidth())
    ...
    def retranslateUi(self, Principal):
        Principal.setWindowTitle(QtGui.QApplication.translate(
            "Principal", "MySql con ayuda", None, QtGui.QApplication.UnicodeUTF8))
        self.Etiqueta.setText(QtGui.QApplication.translate(
            "Principal", "Usuario:", None, QtGui.QApplication.UnicodeUTF8))
        self.Boton_Salir.setText(QtGui.QApplication.translate(
            "Principal", "&Salir", None, QtGui.QApplication.UnicodeUTF8))
  #}}}1
...
class Ui_DialogoInsertar(object):  # {{{1
    def setupUi(self, DialogoInsertar):
        ...
        DialogoInsertar.resize(QtCore.QSize(QtCore.QRect(0, 0, 400, 300).size(
        )).expandedTo(DialogoInsertar.minimumSizeHint()))
        ...
        sizePolicy.setVerticalStretch(0)
        sizePolicy.setHeightForWidth(
            DialogoInsertar.sizePolicy().hasHeightForWidth())
        ...
        self.botones.setStandardButtons(
        ...
        QtCore.QObject.connect(
            self.botones, QtCore.SIGNAL("accepted()"), DialogoInsertar.accept)
        QtCore.QObject.connect(
            self.botones, QtCore.SIGNAL("rejected()"), DialogoInsertar.reject)
        QtCore.QMetaObject.connectSlotsByName(DialogoInsertar)

    def retranslateUi(self, DialogoInsertar):
        DialogoInsertar.setWindowTitle(QtGui.QApplication.translate(
            "DialogoInsertar", "Dialog", None, QtGui.QApplication.UnicodeUTF8))
  #}}}1
...
class SqlDBase:  # {{{1
    ...
    def __init__(self):  # {{{
        ...
        self.Completados = []
        [self.Completados.append(
            a[9:].lower()) for a in temp if a[:9] == 'complete_']
        self.dbases = []
        self.tablas = []
        self.privilegios = [
            'all', 'alter', 'create', 'tables', 'delete', 'drop', 'execute', 'file', 'index', 'insert', 'privileges', 'temporary', 'process', 'references', 'reload',
            'client', 'databases', 'select', 'shutdown', 'super', 'update', 'usage']
          #}}}

    def ejecutarDB_t(self, comando):  # {{{
        a = getoutput(
            query % ('-t', App.User, App.Contrasena, App.Base, comando))
        if a.find('ERROR') != -1:
            tipo = Error
        else:
            tipo = Normal
    ...
    def ejecutarDB(self, comando):  # {{{
        a = getoutput(
            query % ('-B', App.User, App.Contrasena, App.Base, comando))
        if a.find('ERROR') != -1:
            App.Salida(a, Error)
            return ''
        a = a.split('\n')
...
de nuevos datos."""
    def do_INSERTAR(self):
        ...
        if len(linea) == 1:
            App.Salida(
                'Debes indicarme la tabla en la que quieres introducir los datos.', Error)
        else:
            ...
            for x in range(0, len(loscampos)):
                ...
                self.groupBox.setTitle(elDialogo.tr(
                    loscampos[x].split('.')[1]+' <'+self.tipos[x]+'>'))
                self.combo.append(QtGui.QComboBox(self.groupBox))
                self.combo[-1].setGeometry(QtCore.QRect(10, 20, 361, 25))
                self.combo[-1].setObjectName(elDialogo.tr(
                    loscampos[x].split('.')[1]))
                if self.tipos[x].find('enum') != -1:
                    self.combo[-1].setEditable(False)
                else:
                    self.combo[-1].setEditable(True)
...
class MyApp(QtGui.QApplication):  # {{{1
    def __init__(self):  # {{{2
        ...
        self.Principal.connect(self.ui.Boton_Salir, QtCore.SIGNAL(
            'clicked()'), self, QtCore.SLOT('quit()'))
        self.Principal.connect(
            self.Comando, QtCore.SIGNAL('returnPressed()'), self.Ejecutar)
        ...
        self.Principal.connect(
            ParaAyuda, QtCore.SIGNAL('triggered()'), self.Completar)
        ...
        self.Principal.connect(
            ParaSalir, QtCore.SIGNAL('triggered()'), self.quit)
        ...
        if len(linea) == 0:
            ...
            if len(linea) > 0:
                if linea[0] in self.Sql.Completados:
                    ...
                    temp = [a for a in self.Sql.Comandos if a.startswith(
                        linea[0])]
